    ...
    db.get(doc.brewery_id, function(err, result) {
      if (result.value === undefined) { // Trying to edit non-existing doc ?
        ...
        db.upsert( req.params.beer_id, doc, function(err, doc, meta) {
           if(err) {
              console.log("done_edit_beer error = " + err);
              res.send("ERROR" + err);
              res.end;    
           } else {
              res.send("OK");
...
//    var doc = normalize_beer_fields(req.body);
//    var beer_id = doc.brewery_id.toLowerCase() + '-' + doc.name.replace(' ', '-').toLowerCase();
     var doc = req.body; 
    beer_id = doc.id;
    db.insert( beer_id, doc, function(err, result) {
      if (err) {
         console.log("done_create_beer error = " + err);
         res.send("ERROR" + err);
         res.end;
      }
      else  {
...
//    var value = req.query.value;
  ...
  app.get('/beers/search/:key', search_beer);
...
//function validate_beer_field(data) {
//   function
//}
